...
def main(selection="user", headless=False, short_exec=False):
    ...
    # )
    # scene = InteractiveIndoorScene("Rs_int", load_object_categories=["floors"])
    scene = EmptyScene()
    ...
    s.switch_main_vr_robot(bvr_robot)
    # Main simulation loop
    while True:
        s.step()
        actionStep = s.gen_vr_robot_action()

        bvr_robot.apply_action(actionStep)
