...
from igibson.robots.behavior_robot import BODY_ANGULAR_VELOCITY, BODY_LINEAR_VELOCITY, HAND_BASE_ROTS, EYE_LOC_POSE_TRACKED
    ...
    def gen_vr_robot_action(self):
        ...
        * See BehaviorRobot.py for details on the clipping thresholds for
        ...
        if self.vr_settings.using_tracked_body:
            ...
            if hmd_is_valid:
                des_body_pos, des_body_orn = hmd_pos - np.array(EYE_LOC_POSE_TRACKED[0]), p.getQuaternionFromEuler([0, 0, calc_z_rot_from_right(hmd_r)])
            else:
                des_body_pos, des_body_orn = prev_body_pos, prev_body_orn
